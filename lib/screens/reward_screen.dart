...
class _RewardScreenState extends State<RewardScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Preload Rewarded Ad
    widget.adService.loadRewardedAd().then((loaded) {
      if (kDebugMode) {
        if (loaded) {
          print("RewardScreen: Rewarded Ad preloaded successfully.");
        } else {
          print("RewardScreen: Rewarded Ad failed to preload.");
        }
      }
    });
  }
  ...
  void _watchAd() async {
    ...
    // No need to call loadRewardedAd here as it's preloaded in initState
    // and reloaded in AdService's onAdDismissedFullScreenContent.

    if (kDebugMode) print('RewardScreen: Attempting to show preloaded Rewarded Ad.');
    widget.adService.showRewardedAd(
      onUserEarnedReward: (AdWithoutView ad, RewardItem reward) {
        if (kDebugMode) print('RewardScreen: User earned reward: ${reward.amount}');
        _updateUserBalance(reward.amount.toInt(), 'watched_ad');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('You earned ${reward.amount} points!'),
              backgroundColor: Theme.of(context).colorScheme.secondary,
            ),
          );
        }
      ...
      onAdShowed: () {
        if (kDebugMode) print('RewardScreen: Rewarded Ad showed successfully.');
        // Ad is visible, no specific action needed here for now
      },
      onAdDismissed: () {
        if (kDebugMode) print('RewardScreen: Rewarded Ad dismissed.');
        // AdService handles preloading the next ad
        if (mounted) {
            setState(() => _isLoadingAd = false);
        }
      },
      onAdFailedToShow: (AdError error) {
        if (kDebugMode) print('RewardScreen: Rewarded Ad failed to show: ${error.message}');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Failed to show ad: ${error.message}. Please try again later.'),
