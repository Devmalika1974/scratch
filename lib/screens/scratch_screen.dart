...
class _ScratchScreenState extends State<ScratchScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    widget.adService.loadInterstitialAd().then((loaded) {
      if (kDebugMode) {
        if (loaded) {
          print("ScratchScreen: Interstitial Ad preloaded successfully.");
        } else {
          print("ScratchScreen: Interstitial Ad failed to preload.");
        }
      }
    }); 
  }
