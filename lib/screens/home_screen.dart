...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Pre-load Native Ad
    _adService.loadNativeAd(factoryId: 'listTile').then((ad) {
      if (mounted) {
        setState(() {
          _nativeAd = ad;
      ...
      debugPrint('HomeScreen: Native Ad failed to load: $error');
      if (mounted) {
        setState(() {
  ...
  void _initializeTabController() {
    ...
    _tabController.addListener(() {
      if (_tabController.indexIsChanging) {
        // Only show ad if the tab is actually changing and not on initial load or programmatic changes
        if ((_tabController.index == 0 || _tabController.index == 1) && 
            (_previousTabIndex != 0 && _previousTabIndex != 1) ) {
          _adService.showInterstitialAd();
          _adService.loadInterstitialAd(); // Load next ad
        }
      }
